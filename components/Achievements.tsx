...
const AchievementCard: React.FC<{ icon: React.ReactNode; value: string; label: string; }> = ({ icon, value, label }) => {
    ...
    return (
        <div className="achievement-card text-center p-8 bg-[#1a1a1a]/30 border border-transparent rounded-xl backdrop-blur-sm transition-all duration-300 hover:border-white/20 hover:bg-[#1a1a1a]/60">
            <div className="flex justify-center mb-4">{icon}</div>
            <h3 ref={valueRef} className="font-space-grotesk text-5xl font-bold">{isNumeric ? `0${value.includes('%') ? '%' : ''}` : value}</h3>
...
const Achievements: React.FC = () => {
    const sectionRef = useRef<HTMLElement>(null);
     ...
     useEffect(() => {
        const ctx = gsap.context(() => {
            gsap.fromTo(sectionRef.current?.querySelectorAll('.achievement-card'),
    ...
    return (
        <section ref={sectionRef} id="achievements" className="py-24 px-4">
